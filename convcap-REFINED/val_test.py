...
if __name__ == '__main__':
    ...
    model_dir = './models/'
    model_text = 'model_' + str(8) + '.pth'
    modelfn = osp.join(model_dir, model_text)
    
    model_imgcnn = getVgg16()
    model_imgcnn.cuda() 

    model_convcap = convcap(7541, 3, is_attention=False)
    model_convcap.cuda()
    
    
    checkpoint = torch.load(modelfn)
    model_convcap = model_convcap.load_state_dict(checkpoint['state_dict'])
    model_imgcnn = model_imgcnn.load_state_dict(checkpoint['img_state_dict'])
    
    
    test( model_dir, data_folder, 20, 'TEST', 3, False, 0, modelfn, model_convcap, model_imgcnn)
